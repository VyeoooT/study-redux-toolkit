...
export const blogApi = createApi({
  ...
  baseQuery: fetchBaseQuery({
    ...
    prepareHeaders(headers) {
      headers.set('authorization', 'Bearer laeioryikasdbnvcask')
      return headers
    }
  }),
  endpoints: (build) => ({
     ...
     */
    addPost: build.mutation<PostType, Omit<PostType, 'id'>>({
      query(body) {
        try {
    ...
    updatePost: build.mutation<PostType, { id: string; body: PostType }>({
      query(data) {
        return {
          url: `posts/${data.id}`,
          method: 'PUT',
