...
const initialState: Omit<PostType, 'id'> = {
  title: '',
  description: '',
  featuredImage: '',
  publishDate: '',
...
export default function CreatePost() {
  const [formData, setFormData] = useState<Omit<PostType, 'id'> | PostType>(initialState)
  const [addPost, addPostResult] = useAddPostMutation()
  const [updatePost, updatePostResult] = useUpdatePostMutation()
  const postId = useSelector((state: RootState) => state.blog.postId)
  const { data, refetch } = useGetPostQuery(postId, { skip: !postId, refetchOnMountOrArgChange: 10, pollingInterval: 2000 })
